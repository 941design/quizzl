...
describe('emoji-feature i18n keys', () => {
  it('English copy has all eight emoji keys with exact non-empty string values', () => {
    const en = getCopy('en');
    ...
    expect(en.emoji.couldntReact).toBe("Couldn't react");
    expect(en.emoji.reactors).toBe('Reactors');
    expect(en.emoji.reactionCount).toBe('reactions');
  });

  it('German copy has all eight emoji keys with exact non-empty string values', () => {
    const de = getCopy('de');
    ...
    expect(de.emoji.couldntReact).toBe('Reaktion fehlgeschlagen');
