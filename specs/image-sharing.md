...
2. **Pre-process** in a Web Worker (`new Worker(new URL('./imageProcessingWorker.ts', import.meta.url))`) so the main thread stays responsive:
   - Decode the image with `createImageBitmap`.
   - If width or height exceed `MAX_EDGE` (constant: 2048), scale down preserving aspect ratio. Always re-encode to **WebP at 0.85 quality** via `OffscreenCanvas.convertToBlob`. Re-encoding through a canvas inherently strips EXIF and any embedded ICC/IPTC metadata.
...
### Server
Hardcoded default in `app/src/config/blossom.ts`. Recommend `https://blossom.band` or another large free server; final choice is a follow-up call but the code path is server-agnostic. The constant lives in one file so swapping later is a one-line change.
...
- Failure scenarios: simulate Blossom 500 (intercept the upload route in Playwright) → assert the retry button surfaces; click retry → assert success.
...
- **Blossom server choice.** Hardcoded default needs to be picked. Suggest `https://blossom.band` or `https://cdn.satellite.earth` based on uptime + free-tier policy. Final pick before merge.
...
- **OffscreenCanvas + Web Worker support.** Safari support is recent (16.4+). Fallback path: do the processing on the main thread inside `requestIdleCallback` and accept the brief jank. Detect at module load.
...
**New:**
- `app/src/lib/media/imageProcessing.ts`
- `app/src/lib/media/imageProcessingWorker.ts`
