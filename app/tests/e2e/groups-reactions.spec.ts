 ...
 *
 * Uses the window.__quizzlReactions state-injection bridge (exposed in GroupChat.tsx
...
/** Wait for the __quizzlReactions bridge to become available. */
...
async function sendReactionViaBridge(
  ...
  await page.evaluate(
    ({ gid, mid, emoji, isRemoval }) => {
      return (window as any).__quizzlReactions.send(gid, mid, emoji, isRemoval);
    },
 ...
 * rather than the window.__quizzlReactions bridge. The bridge is preserved in
