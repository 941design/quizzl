...
If a user removes a relay on which their key package (kind 30443) or relay-list was
published, peers may no longer find them to send invites. The feature must, on relay-list
save, **re-publish the user's kind 30051 relay list** (the Marmot relay advertisement the
app already publishes in `MarmotContext`) **on the new relay set, and trigger the existing
key-package discoverability path** so a key package (kind 30443) is present on the new set.
The app already has infrastructure for both; it must re-run them against the new list, not
silently leave stale advertisements. (AC-RELAY-9.)

> **Correction (validator, 2026-06-15):** The app publishes kind **30051** for relay
> advertisement, not kind 10051. Kind 10051 is only *read* from peers in
> `NdkNetworkAdapter.ts:getUserInboxRelays`. Earlier spec drafts cited kind 10051
> incorrectly.
...
3. **NIP-07 browser-extension signer** — a third signer mode (`nip07`) using a desktop
   ...
   **Adapter obligation (validator addition, 2026-06-15):** `NDKNip07Signer` (like
   `NDKNip46Signer`) does not expose a `.nip44 = { encrypt, decrypt }` sub-object on the
   signer instance — it uses `NDKUser` objects, not pubkey hex strings. The
   `applesauce-core` `EventSigner` interface that `MarmotClient` requires expects exactly
   that sub-object shape. Therefore the `nip07` path **must wrap `NDKNip07Signer` in the
   same `EventSigner`-shaped adapter as `nip46`** (routing `.nip44.encrypt/decrypt` calls
   through `window.nostr.nip44.encrypt/decrypt` with hex-pubkey arguments). The adapter is
   distinct from the extension-detection guard (AC-NIP07-1); without the adapter, marmot
   crashes even when `window.nostr.nip44` exists. (AC-NIP07-2.)
...
- **AC-RELAY-9** Saving a new relay list re-publishes the user's **kind 30051** relay list
  (the Marmot relay advertisement) on the new set and triggers the key-package
  discoverability path so a key package (kind 30443) is published there (no stranded
  discoverability). See §4.4 for the kind-10051 correction note.
...
- **AC-NIP07-1** Entering `nip07` mode is **refused with a clear error** when the extension
  ...
  work end to end.
- **AC-NIP07-2** The `nip07` signer path wraps `NDKNip07Signer` in an adapter that exposes
  a `.nip44 = { encrypt, decrypt }` sub-object satisfying the `applesauce-core`
  `EventSigner` interface; calls route to `window.nostr.nip44.encrypt/decrypt` with hex
  pubkey arguments. Without this adapter, marmot crashes even when `window.nostr.nip44`
  is present. (Same adapter obligation as `nip46`; §10 already documents it for NIP-46.)
- **AC-SIGNER-10** After the signer mode switch completes, **both** the NDK signer and the
  marmot `EventSigner` reflect the new mode before the UI indicates readiness (success
  state shown, publish-path enabled). No event can be published between the two bindings
  using mismatched signers. Implementation: the mode switch re-runs `MarmotContext.init()`
  (which rebuilds `MarmotClient` with the new signer and rebinds NDK's signer in the same
  effect pass) so both seams are always consistent when init completes.
