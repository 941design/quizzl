...
- **AC-045** — *[Amended in RECONCILE 2026-05-18.]* E2E spec `app/tests/e2e/groups-profile-request.spec.ts` includes five Playwright scenarios: aged-history backfill (target online), periodic refresh (8 d shift), per-peer dedupe (rumor counts), relay-on-behalf with target offline, and forged-sig rejection. The retry state machine boundary contract (1 h / 7 d / `UNANSWERED_MAX_ATTEMPTS = 3`) is verified deterministically by `app/tests/unit/profileRequestStorage.integration.test.ts` (8 tests over the real `recordRequestEmitted` / `shouldEmitRequest` storage path via `fake-indexeddb` + `idb-keyval`). The original 6th E2E scenario (Playwright fake-clock + page.goto retry) remains `test.fixme` at `groups-profile-request.spec.ts:298` and is tracked as an open follow-up; its scope is environment-specific (Playwright clock + cross-navigation re-mount), not contract verification.
...
- **AC-047** — `payloadToMemberProfile` MUST key the resulting `MemberProfile.pubkeyHex` from `payload.signedEvent.pubkey` when present; the caller-supplied `fallbackPubkeyHex` parameter is used only when `signedEvent` is absent (legacy unsigned payloads). The `MarmotContext.tsx` `PROFILE_RUMOR_KIND` dispatcher arm MUST drop the rumor (no IDB write, no contact-cache update, no score-nickname update) when `signedEvent` is present and `verifyEvent(signedEvent)` returns false. Cross-pubkey relay (where `signedEvent.pubkey !== rumor.pubkey`) MUST be accepted — the embedded-sig verification (`verifyEvent`) is the trust anchor, not outer-pubkey matching. This correctly models relay-on-behalf: receivers key the resulting `MemberProfile` under `signedEvent.pubkey` (the cryptographic author) regardless of which MLS member relayed the rumor. [Amended 2026-05-18 RECONCILE: original second clause incorrectly required dropping on outer/inner pubkey mismatch, which would break relay-on-behalf. The sig-verification gate at `parseProfilePayload` (AC-002/AC-003/AC-040) is the correct and sufficient identity protection.]
