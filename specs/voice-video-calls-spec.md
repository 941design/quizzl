...
Signaling transport is settled (§3, Amethyst-compatible gift-wrap). Remaining product calls:

1. **Whether to also post a group-visible "call started/ended" notice** as a kind-445 MLS
   message (one per call, not per signaling event), so the call appears in the group timeline
   and non-ringing members can knock to join. *Recommended: yes, lifecycle-only.*
2. **Default TURN provider** vs. require operator-supplied TURN. *Recommended: ship a default,
   allow override.*
3. **IP-privacy mode** (`relay`-only ICE) default on or off. *Recommended: off by default,
   user-enableable.*
4. **Ring timeout** value and **missed-call** representation in the UI/history.
