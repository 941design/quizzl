...
- **2026-05-18 — AC-045 rewritten (RECONCILE).** The original AC required six Playwright scenarios in `groups-profile-request.spec.ts`. Scenario 5 (retry state machine) was `test.fixme`'d at line 298 because under Playwright's `page.clock.install()` + `page.goto()` cycling, `memo.attempts` stays at 0 across simulated 2h jumps (real root cause not yet diagnosed; candidate causes are fake-clock + `setInterval` interaction, fake-clock-driven memo update path, or `requestedOnEntryForRef` cycling under navigation). The boundary contract is in fact verified deterministically by `app/tests/unit/profileRequestStorage.integration.test.ts` (8 tests exercising the real `recordRequestEmitted` / `shouldEmitRequest` path via `fake-indexeddb` + `idb-keyval`). AC-045 was rewritten to require five Playwright scenarios + the unit/integration test for the retry boundary. The 6th Playwright scenario remains `test.fixme` with a corrected comment citing the unit test; the Playwright-clock-driven scenario stays open as a follow-up.
- **2026-05-18 — AC-047 second clause corrected (RECONCILE remediation round 1).** The original second clause required dropping profile rumors when `signedEvent.pubkey !== rumor.pubkey`. This was incorrect: that condition describes legitimate relay-on-behalf, which the spec explicitly endorses (see `## Design`, relay-on-behalf framing). The real protection against identity confusion is `verifyEvent(signedEvent)` — a forged profile cannot produce a valid sig. AC-047 has been amended to replace the outer/inner pubkey comparison with a `verifyEvent` gate, and to explicitly state that cross-pubkey relay MUST be accepted. The existing implementation in `handlers/profileHandler.ts` and `parseProfilePayload` already satisfies the amended AC. Regression test added: `app/tests/unit/marmot/profileHandler.test.ts` `relay-on-behalf … is accepted, NOT dropped`.
